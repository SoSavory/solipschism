  ...
  task match_aliases: :environment do
    puts "Running task"
     ...
     Alias.where(effective_date: Date.today).order(:id).pluck(:id).each do |alias_id|
      ...
      compare_aliases = Alias.where(effective_date: Date.today).includes(:matched_aliases, :coordinates).references(:matched_aliases, :coordinates).where("aliases.id NOT IN (?)", already_matched_aliases).pluck('aliases.id, coordinates.latitude, coordinates.longitude')
